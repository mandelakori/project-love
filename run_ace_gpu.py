...
# ---------------------------------------------------------------------
# Helper: choose GPU parameters when a CUDA device is present
# ---------------------------------------------------------------------
def get_xgb_params(base_params: dict) -> dict:
    """Add GPU options to XGBoost parameters if a GPU is available.

    The function attempts a lightweight `nvidia-smi` call; if it succeeds we
    inject ``tree_method='gpu_hist'`` and ``predictor='gpu_predictor'``.
    If no GPU is found we simply return the original dict unchanged.
    """
    gpu_available = False
    try:
        # ``nvidia-smi -L`` lists GPUs; a zero‑exit code means the driver is present.
        import subprocess
...
def load_atp_data():
    ...
    if not files:
        raise FileNotFoundError("No ATP match CSVs found in data/. Run 'python ingest_data.py' first.")
    dfs = []
    for f in files:
        try:
...
def enrich_match_ranks(df, ranking_df):
    ...
    winner_left = winner_left.dropna(subset=["player_id", "tourney_date"]).astype({"player_id": "int64", "tourney_date": "int64"})
    ...
    loser_left = loser_left.dropna(subset=["player_id", "tourney_date"]).astype({"player_id": "int64", "tourney_date": "int64"})
...
def get_player_live_features(df, player_name, final_ratings):
    ...
    if not w_rows.empty:
        ...
        rank = int(rnk) if not pd.isna(rnk) else 200
        rnkpts = last.get("winner_rank_points")
        rank_points = float(rnkpts) if not pd.isna(rnkpts) else 0.0
...
# ---------------------------------------------------------------------
# 7. TIME‑SERIES CV (GPU‑aware)
# ---------------------------------------------------------------------
def evaluate_time_series_cv(X, y):
    """Evaluate time‑series cross‑validation by year, using GPU if possible."""
    ...
    for i in range(1, len(years)):
        ...
        model = xgb.XGBClassifier(**get_xgb_params(base_params))
...
def main():
    print("=========================================")
    print("      Project ACE - Unified Engine (GPU)       ")
    print("=========================================")
    ...
    if os.path.exists(model_path):
        ...
        try:
            ...
            model = None
    if model is None:
        print("    [Model] Training model from scratch using optimized parameters (GPU)…")
        base_params_opt = {
            "max_depth": 3,
        ...
        model = xgb.XGBClassifier(**get_xgb_params(base_params_opt))
